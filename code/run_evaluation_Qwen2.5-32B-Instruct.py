...
PAI_EAS_BASE_URL = "http://1092663981048974.cn-hangzhou.pai-eas.aliyuncs.com/api/predict/quickstart_deploy_20260129_wfgx/v1"
# 初始化 PAI-EAS 客户端
try:
...
# ==================== [配置：选择要测试的模型] ====================
# 设置为模型名称（如 "Qwen2.5-32b-Instruct"）来只测试单个模型，设置为 None 则并发测试所有模型
MODEL_TO_TEST = "Qwen2.5-32b-Instruct" # 可选值: None, "Qwen2.5-32b-Instruct"

# 定义要测试的模型列表（与 PVQ.py 中的模型一致）
SUBJECT_MODELS = [
    {"id": "Qwen2.5-32b-Instruct", "name": "Qwen2.5-32b-Instruct", "reasoning": True, "api_type": "pai_eas"}
]
...
def call_subject_model_api(model_config, prompt_text, temperature, system_prompt="You are a helpful assistant.", max_tokens=1024, retries=3, delay=5):
    ...
    if api_type == "pai_eas":
        ...
        if pai_eas_client is None:
            return "PAI_EAS_CLIENT_NOT_INITIALIZED"
        ...
        for attempt in range(retries):
            try:
                ...
                print(f"  [PAI-EAS API Error] Attempt {attempt + 1}/{retries} failed: {e}")
                if attempt < retries - 1:
                    time.sleep(delay)
                else:
                    return "API_ERROR"
