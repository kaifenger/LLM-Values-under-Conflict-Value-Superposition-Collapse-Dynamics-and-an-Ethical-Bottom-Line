...
MODELS_TO_EVALUATE = [
    {"id": "openai/gpt-5.2", "name": "gpt-5.2", "reasoning": False, "api_type": "openrouter"},
    {"id": "anthropic/claude-sonnet-4.5", "name": "claude-sonnet-4.5", "reasoning": False, "api_type": "openrouter"},
    {"id": "google/gemini-3-pro-preview", "name": "gemini-3-pro-preview", "reasoning": True, "api_type": "openrouter"},
    {"id": "deepseek/deepseek-v3.2", "name": "deepseek-v3.2", "reasoning": True, "api_type": "openrouter"},
    {"id": "qwen3-max", "name": "Qwen3-Max", "reasoning": False, "api_type": "dashscope"},
    {"id": "doubao-seed-1-6-251015", "name": "DouBao-Seed-1.6", "reasoning": False, "api_type": "ark"}
]

# 添加一个选项，用于指定只测试某些模型
SPECIFIC_MODELS = None  # 只测试 Qwen3-Max 和 DouBao-Seed-1.6
...
print(f"[成功] 将通过OpenRouter API调用模型")
...
"""
    ...
    if api_type == "dashscope":
        ...
        # 火山引擎 ARK API 配置
        api_url = f"{ARK_BASE_URL}/chat/completions"  # 使用标准 chat completions 端点
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {ARK_API_KEY}",
        }
    ...
    for attempt in range(retries):
        try:
            ...
            if "choices" in response_json and len(response_json["choices"]) > 0:
                ...
                print(f"  [Claude API Response Error] Unexpected JSON structure: {response_json}")
            ...
            if attempt < retries - 1:
                wait_time = delay * (attempt + 1)  # 逐渐增加等待时间
